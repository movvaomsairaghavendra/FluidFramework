...
import { CursorDirection, IViewCursor } from "./layout";
...
function showPresence(
	...
	if (!presenceInfo.cursor) {
		presenceInfo.cursor = new FlowCursor(
			lineContext.flowView.viewportDiv,
			presenceInfo.xformPos,
		);
	...
	if (lineContext.deferredAttach) {
		...
		if (lineContext.span) {
			...
			if (lineContext.lineDiv.indentWidth !== undefined) {
				if (!presenceInfo) {
					lineContext.flowView.cursor.assignToLine(
						lineContext.lineDiv.indentWidth,
						lineContext.lineDivHeight,
						lineContext.lineDiv,
					);
				} else {
...
function endRenderSegments(marker: MergeTree.Marker) {
	return (
		...
		(marker.hasRangeLabel("cell") && marker.refType & MergeTree.ReferenceType.NestEnd)
	);
}
...
function renderSegmentIntoLine(
	...
	if (MergeTree.TextSegment.is(segment)) {
		...
		if (
			lineContext.lineDiv.endPGMarker &&
			lineContext.lineDiv.endPGMarker.properties!.header
		) {
		...
		if (
			lineContext.flowView.cursor.pos >= textStartPos &&
			lineContext.flowView.cursor.pos <= textEndPos
		) {
...
function findLineDiv(pos: number, flowView: FlowView, dive = false) {
	return flowView.lineDivSelect(
		(elm) => {
			if (elm.linePos! <= pos && elm.lineEnd! > pos) {
				return elm;
			}
		},
		flowView.viewportDiv,
		dive,
	);
}
...
function decorateLineDiv(lineDiv: ILineDiv, lineFontstr: string, lineDivHeight: number) {
	...
	const symbolDiv = makeContentDiv(
		new ui.Rectangle(
			lineDiv.indentWidth! - Math.floor(em + symbolWidth),
			0,
			symbolWidth,
			lineDivHeight,
		),
...
function reRenderLine(lineDiv: ILineDiv, flowView: FlowView) {
	if (lineDiv) {
		const outerViewportBounds = ui.Rectangle.fromClientRect(
			flowView.viewportDiv.getBoundingClientRect(),
		);
		...
		if (lineDiv.indentWidth) {
			contentDiv = makeContentDiv(
				new ui.Rectangle(lineDiv.indentWidth, 0, lineDiv.contentWidth!, lineDivHeight),
				lineDiv.style.font,
			);
		...
		flowView.sharedString.walkSegments(
			renderSegmentIntoLine,
			lineDiv.linePos,
			end,
			lineContext,
		);
...
function makeContentDiv(r: ui.Rectangle, lineFontstr) {
	...
	contentDiv.onclick = (e) => {
		...
		if (targetDiv.lastElementChild) {
			...
			console.log(
				`div click at ${e.clientX},${e.clientY} rightmost span with text ${targetDiv.lastElementChild.innerHTML}`,
			);
		}
	};
...
function isInnerCell(cellView: ICellView, layoutInfo: ILayoutContext) {
	return (
		!layoutInfo.startingPosStack ||
		!layoutInfo.startingPosStack.cell ||
...
function layoutCell(cellView: ICellView, layoutInfo: ILayoutContext) {
	...
	const cellViewportRect = new ui.Rectangle(
		layoutInfo.docContext.cellHMargin,
		0,
		cellViewportWidth,
		0,
	);
	...
	cellView.viewport = new Viewport(
		layoutInfo.viewport.remainingHeight(),
	...
	if (isInnerCell(cellView, layoutInfo)) {
		...
		cellLayoutInfo.startPos = getPosition(
			layoutInfo.flowView.sharedString,
			nextTable as MergeTree.Marker,
		);
	...
	cellView.borderRect = createSVGRect(
		new ui.Rectangle(0, 0, cellRect.width, cellView.renderedHeight),
	);
...
function renderTable(
	...
	if (!tableView) {
		tableView = Table.parseTable(
			table,
			tablePos,
			flowView.sharedString,
			makeFontInfo(docContext),
		);
	}
	if (!tableView) {
		return;
	...
	if (layoutInfo.startingPosStack) {
		if (
			layoutInfo.startingPosStack.row &&
			layoutInfo.startingPosStack.row.items.length > layoutInfo.stackIndex!
		) {
			const startRowMarker = layoutInfo.startingPosStack.row.items[
				layoutInfo.stackIndex!
			] as Table.IRowMarker;
		...
		if (
			...
			const startCellMarker = layoutInfo.startingPosStack.cell.items[
				layoutInfo.stackIndex!
			] as Table.ICellMarker;
	...
	for (let rowIndex = 0, rowCount = tableView.rows.length; rowIndex < rowCount; rowIndex++) {
		...
		const renderRow =
			deferredHeight >= layoutInfo.deferUntilHeight! &&
			foundStartRow &&
			!Table.rowIsMoribund(rowView.rowMarker);
		...
		let cellX = 0;
		for (
			let cellIndex = 0, cellsLen = rowView.cells.length;
			cellIndex < cellsLen;
			cellIndex++
		) {
...
function showCell(pos: number, flowView: FlowView) {
	...
	if (startingPosStack.cell && !startingPosStack.cell.empty()) {
		...
		// eslint-disable-next-line max-len
		console.log(
			`cell ${cellMarker.getId()} seq ${cellMarker.seq} clid ${cellMarker.clientId} at [${start},${end})`,
		);
...
function showTable(pos: number, flowView: FlowView) {
	...
	if (startingPosStack.table && !startingPosStack.table.empty()) {
		...
		console.log(
			`table contents: ${flowView.sharedString.getTextRangeWithMarkers(start, end)}`,
		);
	}
}
...
function renderTree(
	...
	const startingPosStack = flowView.sharedString.getStackContext(requestedPosition, [
		"table",
		"cell",
		"row",
	]);
...
function makeFontInfo(docContext: IDocumentContext): Paragraph.IFontInfo {
	...
	const glh = (fontstr: string, lineHeight?: string) =>
		domutils.getLineHeight(fontstr, lineHeight);
...
function breakPGIntoLinesFFVP(
	...
	for (let i = 0, len = items.length; i < len; i++) {
		...
		if (item.type !== Paragraph.ParagraphItemType.Marker) {
			committedItemsHeight = Math.max(
				committedItemsHeight,
				item.height ? item.height : defaultLineHeight,
			);
		}
	}
...
function renderFlow(layoutContext: ILayoutContext): IRenderOutput {
	...
	function renderPG(
		...
		for (let breakIndex = 0, len = pgBreaks.length; breakIndex < len; breakIndex++) {
			...
			const lineEnd =
				breakIndex < len - 1 ? pgBreaks[breakIndex + 1].posInPG + pgStartPos : undefined;
			...
			if (lineOK && (lineEnd === undefined || lineEnd > layoutContext.requestedPosition!)) {
				lineDiv = makeLineDiv(
					new ui.Rectangle(lineX, lineY, lineWidth, lineHeight),
					lineFontstr,
				);
				...
				if (indentWidth > 0) {
					contentDiv = makeContentDiv(
						new ui.Rectangle(indentWidth, 0, contentWidth, lineDivHeight),
						lineFontstr,
					);
				...
				const lineContext = {
					contentDiv,
					deferredAttach: layoutContext.deferredAttach,
					flowView: layoutContext.flowView,
					lineDiv,
					lineDivHeight,
					pgMarker: endPGMarker,
					span,
				} as ILineContext;
	...
	do {
		...
		const asMarker = MergeTree.Marker.is(segoff.segment) ? segoff.segment : undefined;
		...
		if (asMarker && asMarker.hasRangeLabel("table")) {
			...
			const endTablePos = getPosition(
				layoutContext.flowView.sharedString,
				tableView.endTableMarker,
			);
			...
			if (currentPos < totalLength) {
				...
				if (currentPos < totalLength) {
					...
					if (MergeTree.Marker.is(segoff.segment)) {
						// eslint-disable-next-line max-len
						if (
							segoff.segment.hasRangeLabel("cell") &&
							segoff.segment.refType & MergeTree.ReferenceType.NestEnd
						) {
							break;
						}
					}
...
class FlowCursor extends Cursor {
	...
	private makePresenceDiv() {
		...
		this.presenceDiv.innerText = this.getUserDisplayString(
			this.presenceInfo!.user as IFlowViewUser,
		);
...
export class FlowView extends ui.Component {
	...
	constructor(
		...
		sharedString.on("sequenceDelta", (event, target) => {
			...
			this.modifiedMarkers = event.ranges.filter((range) =>
				MergeTree.Marker.is(range.segment),
			);
		...
		const commandBoxElement = React.createElement(CommandBox, {
			registerShowListener,
			dismissCallback: onCommandBoxDismiss,
			commands: commandBoxCommands,
		});

		ReactDOM.render(commandBoxElement, commandBoxDiv);
	}
	...
	private updatePresencePosition(localPresenceInfo: ILocalPresenceInfo) {
		if (localPresenceInfo) {
			localPresenceInfo.xformPos = getLocalRefPos(
				this.sharedString,
	...
	private updatePresenceVector(localPresenceInfo: ILocalPresenceInfo) {
		...
		if (
			localPresenceInfo.xformPos !== tempXformPos ||
			localPresenceInfo.markXformPos !== tempMarkXformPos
		) {
			const sameLine =
				localPresenceInfo.cursor &&
			...
			if (elm.tagName === "DIV") {
				if (targetLineDiv.lineEnd! - targetLineDiv.linePos === 1) {
					...
					if (targetLineDiv.indentWidth !== undefined) {
						...
						position =
							relX <= targetLineDiv.indentWidth
								? targetLineDiv.linePos
								: targetLineDiv.lineEnd;
					} else {
					...
					// Content div
					position =
						x <= targetLineBounds.left ? targetLineDiv.linePos : targetLineDiv.lineEnd;
				}
				...
				if (e.ctrlKey && e.keyCode !== 17) {
					...
					if (this.cursor.pos < maxPos || lineCount < 0) {
						if (!this.verticalMove(lineCount)) {
							if (
								(this.viewportStartPos! > 0 && lineCount < 0) ||
	...
	private setList(listKind = 0) {
		...
		if (tileInfo) {
			...
			if (listStatus) {
				...
				this.sharedString.annotateRange(tileInfo.pos, tileInfo.pos + 1, {
	...
	private tryMoveCell(pos: number, shift = false) {
		const cursorContext = this.sharedString.getStackContext(pos, ["table", "cell", "row"]);
	...
	private toggleRange(
		...
		this.sharedString.walkSegments(
			findPropSet as (segment: MergeTree.ISegment) => boolean,
			start,
			end,
		);
		this.undoRedoManager.closeCurrentOperation();
		...
		if (stack.table && !stack.table.empty()) {
			...
			if (!tableMarker.table) {
				...
				Table.parseTable(
					...
					tableMarkerPos,
					this.sharedString,
					makeFontInfo(this.lastDocContext!),
				);
			}
	...
	public loadFinished(clockStart = 0) {
		...
		if (clockStart > 0) {
			// eslint-disable-next-line max-len
			console.log(
				`time to edit/impression: ${this.timeToEdit} time to load: ${Date.now() - clockStart}ms len: ${this.sharedString.getLength()} - ${performance.now()}`,
			);
		}
	...
	private remotePresenceToLocal(
		clientId: string,
		remotePresenceInfo: IRemotePresenceInfo,
		posAdjust = 0,
	) {
		...
		if (segoff.segment) {
			...
			if (clientInfo) {
				const localPresenceInfo = {
					...
					shouldShowCursor: () =>
						this.runtime.clientId !== clientId &&
						this.getRemoteClientInfo(clientId) !== undefined,
					user: clientInfo.user,
				} as ILocalPresenceInfo;
				if (remotePresenceInfo.origMark >= 0) {
					const markSegoff = this.sharedString.getContainingSegment(
						remotePresenceInfo.origMark,
					);
					if (markSegoff.segment) {
						localPresenceInfo.markLocalRef = this.sharedString.createPositionReference(
							markSegoff.segment,
							markSegoff.offset,
							MergeTree.ReferenceType.SlideOnRemove,
						);
	...
	private increaseIndent(tile: Paragraph.IParagraphMarker, pos: number, decrease = false) {
		...
		if (decrease && tile.properties!.indentLevel > 0) {
			this.sharedString.annotateRange(
				...
				pos + 1,
				{ indentLevel: -1 },
				{ name: "incr", defaultValue: 1, minValue: 0 },
			);
		} else if (!decrease) {
			this.sharedString.annotateRange(
				pos,
	...
	private handleSharedStringDelta(
		event: Sequence.SequenceDeltaEvent,
		target: Sequence.SharedString,
	) {
		...
		if (event.isLocal) {
			...
			this.queueRender(
				undefined,
				this.posInViewport(event.first.position) || this.posInViewport(opCursorPos!),
			);
		}
	}
