...
const getPosition = (sharedString: SharedString, segment: MergeTree.ISegment) =>
	sharedString.getPosition(segment);
...
function createCellBegin(
	...
	opList.push(
		createRelativeMarkerOp(
			cellEndRelPos,
			cellId,
			MergeTree.ReferenceType.NestBegin,
			["cell"],
			undefined,
			startExtraProperties,
		),
	);
	const pgOp = createRelativeMarkerOp(
		cellEndRelPos,
		`${cellId}C`,
...
function createEmptyRowAfter(
	...
	opList.push(
		createRelativeMarkerOp(endRowPos, endPrefix + rowId, MergeTree.ReferenceType.NestEnd, [
			"row",
		]),
	);
	opList.push(
		createRelativeMarkerOp(endRowPos, rowId, MergeTree.ReferenceType.NestBegin, ["row"]),
	);
}
...
function createColumnCellOp(
	...
	createCellRelative(opList, idBase, { id }, { columnId });
}
...
// TODO: GC using consensus remove
export function deleteCellShiftLeft(sharedString: SharedString, cell: Cell, table: Table) {
...
export function createTable(
	...
	opList.push(
		createMarkerOp(
			...
			MergeTree.ReferenceType.NestEnd | MergeTree.ReferenceType.Tile,
			["table"],
			["pg"],
		),
	);
	...
	for (let row = 0; row < nrows; row++) {
		const rowId = `${idBase}row${rowIdSuffix++}`;
		opList.push(
			createRelativeMarkerOp(endTablePos, rowId, MergeTree.ReferenceType.NestBegin, ["row"]),
		);
		...
		opList.push(
			createRelativeMarkerOp(endTablePos, endPrefix + rowId, MergeTree.ReferenceType.NestEnd, [
				"row",
			]),
		);
	}
...
export class Table {
	...
	public nextcell(cell: Cell) {
		...
		for (let rowIndex = 0, rowCount = this.rows.length; rowIndex < rowCount; rowIndex++) {
			...
			for (
				let cellIndex = 0, cellCount = row.cells.length;
				cellIndex < cellCount;
				cellIndex++
			) {
...
export function succinctPrintTable(
	tableMarker: ITableMarker,
	tableMarkerPos: number,
	sharedString: SharedString,
) {
