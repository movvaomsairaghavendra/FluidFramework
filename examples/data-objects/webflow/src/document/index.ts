	...
	if (TextSegment.is(segment)) {
		...
		switch (markerType) {
			...
			case ReferenceType.Tile | ReferenceType.NestBegin:
				const kind = (
					segment.hasRangeLabels() ? segment.getRangeLabels()[0] : segment.getTileLabels()[0]
				) as DocSegmentKind;
			...
			default:
				...
				assert(
					segment.getRangeLabels()[0] === DocSegmentKind.beginTags,
...
export class FlowDocument extends LazyLoadedDataObject<ISharedDirectory, IFlowDocumentEvents> {
	...
	public addLocalRef(position: number) {
		...
		if (position >= this.length) {
			return this.sharedString.createPositionReference(
				endOfTextSegment,
				0,
		...
		const localRef = this.sharedString.createPositionReference(
			segment,
			offset,
