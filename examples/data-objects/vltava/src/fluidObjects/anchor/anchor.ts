	...
	implements IProvideFluidHTMLView, IProvideFluidLastEditedTracker
{
	...
	public get IFluidHTMLView() {
		return this.defaultFluidObject;
	}
	...
	protected async initializingFirstTime() {
		...
		const lastEditedFluidObject =
			await LastEditedTrackerDataObject.getFactory().createChildInstance(this.context);
	...
	protected async hasInitialized() {
		...
		this.lastEditedFluidObject = (
			await this.root
				.get<IFluidHandle<IFluidLastEditedTracker>>(this.lastEditedFluidObjectId)
				?.get()
		)?.IFluidLastEditedTracker;
	}
}
