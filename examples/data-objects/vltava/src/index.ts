...
import { Anchor, TabsFluidObject } from "./fluidObjects";
import {
...
export class InternalRegistry
	implements IFluidDataStoreRegistry, IFluidObjectInternalRegistry
{
	public get IFluidDataStoreRegistry() {
		return this;
	}
	public get IFluidObjectInternalRegistry() {
		return this;
	}

	constructor(private readonly containerFluidObjectArray: IInternalRegistryEntry[]) {}

	public async get(
		name: string,
	): Promise<Readonly<IProvideFluidDataStoreFactory | undefined>> {
	...
	public getFromCapability(
		capability: FluidObjectKeys<DefaultRegistryTypes>,
	): IInternalRegistryEntry[] {
		return this.containerFluidObjectArray.filter((fluidObjectDetails) =>
			fluidObjectDetails.capabilities.includes(capability),
		);
	}
	...
	public hasCapability(type: string, capability: FluidObjectKeys<DefaultRegistryTypes>) {
		...
		return (
			index >= 0 && this.containerFluidObjectArray[index].capabilities.includes(capability)
		);
	}
...
export class VltavaRuntimeFactory extends ContainerRuntimeFactoryWithDefaultDataStore {
	...
	protected async containerHasInitialized(runtime: IContainerRuntime) {
		...
		const tracker = await requestFluidObject<IFluidLastEditedTracker>(
			await runtime.getRootDataStore(
...
const generateFactory = () => {
	...
	return new VltavaRuntimeFactory(Anchor.getFactory(), [
