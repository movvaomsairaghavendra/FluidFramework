	...
	constructor(
			...
			new MenuItem({
				...
				run: (state, _, view) => {
					...
					if (
						...
						state.selection.node.type === fluidSchema.nodes.fluid
					) {
		...
		menu.insertMenu.content.push(
			new MenuItem({
				...
				label: "Footnote",
				select: (state) =>
					insertPoint(state.doc, state.selection.from, fluidSchema.nodes.footnote) != null,
				run(state, dispatch) {
					...
					dispatch(
						state.tr.replaceSelectionWith(fluidSchema.nodes.footnote.create(null, content)),
					);
				},
			}),
		);
