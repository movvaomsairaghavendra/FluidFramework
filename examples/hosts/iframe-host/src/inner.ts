...
import { InnerDocumentServiceFactory, InnerUrlResolver } from "@fluidframework/iframe-driver";
...
async function loadFluidObject(divId: string, container: IContainer) {
...
async function attachContainer(containerId: string, request: IRequest): Promise<void> {
...
export async function runInner(divId: string) {
	const innerApi: IFrameInnerApi = {
		...
		loadContainer: Comlink.proxy(async (documentId, createNew) =>
			loadContainer(documentId, createNew, "content"),
		),
		attachContainer: Comlink.proxy(async (containerId, request) =>
			attachContainer(containerId, request),
		),
	};
