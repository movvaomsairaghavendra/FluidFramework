...
import { IFrameInnerApi, IFrameOuterHost } from "./inframehost";
...
export async function loadFrame(
	...
	iframe.addEventListener("load", function loadFn() {
		void (async () => {
			...
			const innerApi = Comlink.wrap<IFrameInnerApi>(
...
async function loadOuterLogDiv(container: IContainer, logDivId: string): Promise<void> {
	...
	quorum.getMembers().forEach((client) => {
		logDiv.innerHTML += `Quorum: client: ${JSON.stringify(client)}<br/>`;
	});
 ...
 */
async function loadOuterDataStoreDiv(dataStoreDivId: string): Promise<void> {
