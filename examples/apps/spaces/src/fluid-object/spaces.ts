...
export class Spaces extends DataObject {
	...
	public readonly getBaseUrl = async () =>
		this.context.getAbsoluteUrl(this.handle.absolutePath);

	protected async initializingFirstTime() {
		const storageComponent = await SpacesStorage.getFactory().createChildInstance(
			this.context,
		);
	...
	protected async hasInitialized() {
		this._storageComponent = await this.root
			.get<IFluidHandle<SpacesStorage>>(SpacesStorageKey)
			?.get();
	}
	...
	public saveLayout(): void {
		localStorage.setItem(
			"spacesTemplate",
			JSON.stringify([...this.storageComponent.itemList.values()]),
		);
	}
	...
	public async setTemplate(): Promise<void> {
		...
		if (templateString) {
			...
			const promises = templateItems.map(async (templateItem) => {
				return this.createAndStoreItem(
					templateItem.serializableItemData.itemType,
					templateItem.layout,
				);
			});
