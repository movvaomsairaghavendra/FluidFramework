...
const SpacesEditPane: React.FC<ISpacesEditPaneProps> = (
	...
	return (
		<div className="spaces-edit-pane">
			<SpacesEditButton title="Delete" clickCallback={props.removeItem}>
				❌
			</SpacesEditButton>
			<SpacesEditButton
...
const SpacesItemView: React.FC<ISpacesItemViewProps> = (
	...
	return (
		<div className="spaces-item-view">
			{props.editable && <SpacesEditPane url={props.url} removeItem={props.removeItem} />}
			<div className="spaces-embedded-item-wrapper">{itemView}</div>
		</div>
	);
};
...
export const SpacesStorageView: React.FC<ISpacesStorageViewProps> = (
	...
	const [itemMap, setItemMap] = React.useState<Map<string, ISpacesStoredItem<any>>>(
		props.storage.itemList,
	);
