...
async function generateMonoRepoPackageLockJson(monoRepo: MonoRepo, repoPackageJson: any) {
	...
	const markNonDev = (name: string, topRef: string, item: any, refStack: any[]) => {
		...
		for (const req in item.requires) {
			if (
				!refStack.some((scope) => scope.dependencies && scope.dependencies[req] !== undefined)
			) {
	...
	console.log(
		`${monoRepo.kind}: ${format(topLevelDevCount)}/${format(topLevelTotalCount)} top level locked devDependencies`,
	);
...
async function generateMonoRepoInstallPackageJson(monoRepo: MonoRepo) {
	...
	console.log(
		`${monoRepo.kind}: ${format(devDepCount)}/${format(depCount + devDepCount)} devDependencies`,
	);
