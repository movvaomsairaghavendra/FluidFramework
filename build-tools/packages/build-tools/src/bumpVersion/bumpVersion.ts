...
export async function bumpRepo(
	...
	for (const pkg of packageNeedBump) {
		...
		// Translate the versionBump into the appropriate change for virtual patch versioning
		const translatedVersionBump = await adjustVersion(
			getVersion(pkg.name),
			versionBump,
			scheme,
		);
		...
		for (const { name, version, dev } of pkg.combinedDependencies) {
			...
			if (depPackage) {
				...
				if (typeof versionBump === "string") {
					dep[name] =
						`npm:${packageName}@^${semver.major(depPackage.version)}.${semver.minor(depPackage.version) - frequency + 1}.0${suffix}`;
				} else {
					dep[name] =
						`npm:${packageName}@^${versionBump.major}.${versionBump.minor - frequency}.0${suffix}}`;
				}
			}
		}
		pkg.savePackageJson();
