...
export class FluidPackageCheck {
	...
	private static checkLintScripts(pkg: Package, fix: boolean) {
		...
		if (pkg.getScript("build")) {
			...
			const hasFixType =
				eslintFixScript && eslintFixScript.search("--fix-type problem,suggestion,layout") >= 0;
			if (
				this.checkScript(
					pkg,
					"eslint:fix",
					`${expectedEslintScript} --fix${hasFixType ? " --fix-type problem,suggestion,layout" : ""}`,
					fix,
				)
			) {
				fixed = true;
			...
			if (commonConfigFullPath) {
				...
				if (commonConfigJson) {
					...
					for (const option in configJson.compilerOptions) {
						if (
							configJson.compilerOptions[option] === commonConfigJson.compilerOptions[option]
						) {
							this.logWarn(
								pkg,
	...
	private static async checkOneTestDir(pkg: Package, fix: boolean, subDir: string) {
		...
		if (!configJson.compilerOptions) {
			...
			if (
				this.checkProperty(configFile, pkg, configJson.compilerOptions, "outDir", outDir, fix)
			) {
				changed = true;
			}
		}
