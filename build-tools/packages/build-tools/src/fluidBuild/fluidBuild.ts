...
async function main() {
	...
	if (options.clean || options.build !== false) {
		logStatus(
			`Symlink in ${options.fullSymlink ? "full" : options.fullSymlink === false ? "isolated" : "non-dependent"} mode`,
		);
		...
		if (options.build !== false) {
			...
			if (commonOptions.timer) {
				...
				logStatus(
					`Execution time: ${totalElapsedTime.toFixed(3)}s, Concurrency: ${concurrency.toFixed(3)}`,
				);
