...
export class BuildGraph {
	...
	public async build(timer?: Timer): Promise<BuildResult> {
		...
		logStatus(
			`Starting npm script "${chalk.cyanBright(this.buildScriptNames.join(" && "))}" for ${this.buildPackages.size} packages, ${this.buildContext.taskStats.leafTotalCount} tasks`,
		);
		if (isUpToDate) {
	...
	private populateLevel() {
		...
		const getLevel = (node: BuildPackage, parent?: BuildPackage) => {
			if (node.level === -2) {
				throw new Error(
					`Circular Reference detected ${parent ? parent.pkg.nameColored : "<none>"} -> ${node.pkg.nameColored}`,
				);
			}
			if (node.level !== -1) {
				return node.level;
