		...
		const execCleanScript = async (pkg: Package, cleanScript: string) => {
			...
			const result = await execWithErrorAsync(
				cleanScript,
				{
					cwd: pkg.directory,
					env: {
						PATH: `${process.env["PATH"]}${path.delimiter}${path.join(pkg.directory, "node_modules", ".bin")}`,
					},
				},
				pkg.nameColored,
			);
			...
			if (status) {
				const elapsedTime = (Date.now() - startTime) / 1000;
				logStatus(
					`[${++numDone}/${cleanP.length}] ${pkg.nameColored}: ${cleanScript} - ${elapsedTime.toFixed(3)}s`,
				);
			}
			return result;
		};
