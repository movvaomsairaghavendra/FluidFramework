...
export class LayerGraph {
	...
	public generateDotGraph() {
		...
		const dotGraph = `strict digraph G { graph [ newrank=true; ranksep=2; compound=true ]; ${this.groupNodes.map((group) => group.generateDotSubgraph()).join("")}
  ${dotEdges.join("\n  ")}
}`;
		return dotGraph;
	}
