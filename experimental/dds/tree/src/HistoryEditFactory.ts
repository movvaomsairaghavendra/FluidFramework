	...
	for (const change of changes) {
		...
		switch (change.type) {
			case ChangeTypeInternal.Build: {
				...
				assert(!builtNodes.has(destination), 0x374 /* Cannot revert Build: destination is already used by a Build */);
				assert(!detachedNodes.has(destination), 0x375 /* Cannot revert Build: destination is already used by a Detach */);
				builtNodes.set(
					destination,
