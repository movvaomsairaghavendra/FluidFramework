...
describe("SharedOT", () => {
	describe("stress", () => {
		...
		async function stress(
			...
			try {
				...
				const insert = (docIndex: number, position: number, text: string) => {
					...
					trace?.push(
						`doc${docIndex + 1}.insert(/* position: */ ${position}, /* text: */ ${JSON.stringify(text)});`,
					);
		...
		it.skip("stress-loop", // Note: Must use 'function' rather than arrow '() => { .. }' in order to set 'this.timeout(..)'
		async function () {
			...
			while (true) {
				...
				if (now - lastStatus > 5000) {
					...
					process.stdout.write(
						`Stress loop: ${iterations} iterations completed - Total Elapsed: ${((Date.now() - start) / 1000).toFixed(2)}s\n`,
					);
					lastStatus = now;
				}
			}
