	...
	implements IContainer
{
	...
	private async load(
		...
		if (!this.closed) {
			if (opsBeforeReturnP !== undefined) {
				...
				await ReportIfTooLong(this.mc.logger, "WaitOps", async () => {
					await opsBeforeReturnP;
					return {};
				});
				await ReportIfTooLong(this.mc.logger, "WaitOpProcessing", async () =>
					this._deltaManager.inbound.waitTillProcessingDone(),
				);
	...
	private async connectStorageService(): Promise<void> {
		...
		this._storageService = new RetriableDocumentStorageService(storageService, this.mc.logger);

		if (this.options.summarizeProtocolTree === true) {
			this.mc.logger.sendTelemetryEvent({ eventName: "summarizeProtocolTreeEnabled" });
			this._storageService = new ProtocolTreeStorageService(this._storageService, () =>
		...
		assert(
			storageService.policies?.minBlobSize === this.storageService.policies?.minBlobSize,
	...
	private submitContainerMessage(
		...
		switch (outboundMessageType) {
			...
			default:
				this.close(
					new GenericError("invalidContainerSubmitOpType", undefined /* error */, {
						messageType: type,
					}),
				);
				return -1;
