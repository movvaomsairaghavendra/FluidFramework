    ...
    private ensureProcessing() {
        if (this.anythingToProcess() && this.processingPromise === undefined) {
            ...
            this.processingPromise = Promise.resolve().then(() => {
                assert(this.processingPromise !== undefined, 0x3bb /* reentrancy? */);
                const result = this.processDeltas();
                assert(this.processingPromise !== undefined, 0x3bc /* reentrancy? */);
            ...
            assert(this.processingPromise !== undefined, 0x3bd /* processDeltas() should run async */);
        }
    }
