...
export class ConnectionManager implements IConnectionManager {
	...
	public shouldJoinWrite(): boolean {
		...
		return (
			this.clientSequenceNumberObserved < this.clientSequenceNumber - this.trailingNoopCount
		);
	}
	...
	public dispose(error?: ICriticalContainerError) {
		...
		const disconnectReason =
			error !== undefined ? `Closing DeltaManager (${error.message})` : "Closing DeltaManager";
	...
	private async connectCore(connectionMode?: ConnectionMode): Promise<void> {
		...
		let requestedMode =
			connectionMode ?? pendingConnectionMode ?? this.defaultReconnectionMode;
