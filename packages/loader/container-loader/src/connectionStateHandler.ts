...
export class ConnectionStateHandler {
	...
	public receivedConnectEvent(connectionMode: ConnectionMode, details: IConnectionDetails) {
		...
		assert(
			writeConnection || !this.handler.shouldClientJoinWrite(),
			0x30a /* shouldClientJoinWrite should imply this is a writeConnection */,
		);
		assert(
			writeConnection || !this.waitingForLeaveOp,
		...
		const waitingForJoinOp =
			writeConnection && quorumClients?.getMember(this._pendingClientId) === undefined;
