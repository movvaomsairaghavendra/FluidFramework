...
export class ScriptManager {
	...
	public async preCacheFiles(browser: IFluidBrowserPackageEnvironment): Promise<void> {
		...
		for (const targetName of Object.keys(browser).filter((t) =>
			precacheTargetList.includes(t),
		)) {
			...
			if (target !== undefined) {
				for (const url of target.files) {
					...
					cacheLink.rel =
						cacheLink.relList?.supports("preload") === true ? "preload" : "prefetch";
	...
	public async loadScript(scriptUrl: string, entryPoint: string): Promise<unknown> {
		return ScriptManager.protectEntrypoint(entryPoint, async () =>
			ScriptManager.internalLoadScript(scriptUrl, entryPoint),
		);
	}

	public async loadLibrary(libraryDetails: { files: string[]; library: string }): Promise<
		{ file: string; entryPoint: unknown }[]
	> {
