		...
		it("resolve should be retryable", async () => {
			axiosMock = {
				post: async <T = any, R = AxiosResponse<T>>() => {
					...
					const r: Partial<AxiosResponse<IResolvedUrl>> = {
						data: { type: "web", data: "http://resolved" },
					};
					return r as R;
				},
			};
			...
			await assert.rejects(
				...
				"resolving should fail if Axios.post fails",
			);
			axiosMock.failPost = false;
			const resolvedUrl = await resolver.resolve(request);
			assert(
				resolvedUrl.type === "web" && resolvedUrl.data === "http://resolved",
