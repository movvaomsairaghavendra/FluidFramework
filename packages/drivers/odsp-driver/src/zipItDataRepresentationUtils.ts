...
export function getAndValidateNodeProps(
	node: NodeCore,
	props: string[],
	enforceAllProps = true,
) {
...
export abstract class BlobCore {
	 ...
	 */
	constructor(
		public readonly constString: boolean,
		public readonly useUtf8Code: boolean = false,
	) {}
...
export const addStringProperty = (
	...
	a: string,
	b: string,
	encodeValAsConstString: boolean = false,
) => {
		...
		for (; !buffer.eof; ) {
			...
			switch (code) {
				...
				case MarkerCodes.ConstStringDeclareBig: {
					...
					const constString = BlobShallowCopy.read(
						buffer,
						getValueSafely(codeToBytesMap, code),
						true,
					);
				...
				case MarkerCodes.BinarySingle64: {
					childValue = BlobShallowCopy.read(
						buffer,
						getValueSafely(codeToBytesMap, code),
						false,
					);
