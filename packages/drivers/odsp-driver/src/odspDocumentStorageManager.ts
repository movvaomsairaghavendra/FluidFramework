...
import { ISummaryContext, DriverErrorType } from "@fluidframework/driver-definitions";
...
export class OdspDocumentStorageService extends OdspDocumentStorageServiceBase {
	...
	public async getVersions(
		...
		if (
			...
			count === 1 &&
			(blobid === null || blobid === this.documentId)
		) {
			...
			const odspSnapshotCacheValue: ISnapshotContents = await PerformanceEvent.timedExecAsync(
				...
				async (event: PerformanceEvent) => {
					...
					const cachedSnapshotP: Promise<ISnapshotContents | undefined> = this.epochTracker
