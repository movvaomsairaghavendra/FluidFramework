...
function readTreeSection(node: NodeCore) {
	...
	for (let count = 0; count < node.length; count++) {
		...
		const records = getAndValidateNodeProps(
			treeNode,
			["name", "value", "children", "unreferenced"],
			false,
		);
...
function readSnapshotSection(node: NodeTypes): ISnapshotSection {
	...
	const records = getAndValidateNodeProps(node, ["id", "sequenceNumber", "treeNodes"]);
...
export function parseCompactSnapshotResponse(buffer: ReadBuffer): ISnapshotContents {
	...
	const records = getAndValidateNodeProps(
		root,
		["mrv", "cv", "lsn", "snapshot", "blobs", "deltas"],
		false,
	);
	...
	assert(
		parseFloat(snapshotMinReadVersion) >= parseFloat(records.mrv.toString()),
		0x20f /* "Driver min read version should >= to server minReadVersion" */,
	);
	assert(
		parseFloat(records.cv.toString()) >= parseFloat(snapshotMinReadVersion),
		0x210 /* "Snapshot should be created with minReadVersion or above" */,
	);
	assert(
		currentReadVersion === records.cv.toString(),
