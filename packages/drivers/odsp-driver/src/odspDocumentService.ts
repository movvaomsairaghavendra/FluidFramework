	...
	public async connectToDeltaStream(client: IClient): Promise<IDocumentDeltaConnection> {
		...
		return getWithRetryForTokenRefresh<IDocumentDeltaConnection>(async (options) => {
			...
			const [websocketEndpoint, websocketToken, io] = await Promise.all([
				...
				this.socketIoClientFactory().catch(
					annotateAndRethrowConnectionError("socketIoClientFactory"),
				),
			]);
			...
			if (finalWebsocketToken === null) {
				throw this.annotateConnectionError(
					new NonRetryableError("Websocket token is null", OdspErrorType.fetchTokenError, {
						driverVersion,
					}),
					"getWebsocketToken",
					!requestWebsocketTokenFromJoinSession,
