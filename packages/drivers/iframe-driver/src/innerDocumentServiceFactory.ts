...
export class InnerDocumentServiceFactory implements IDocumentServiceFactory {
	public static async create(outerPort: MessagePort): Promise<InnerDocumentServiceFactory> {
		...
		const outerProxy = combinedProxy[
			IDocumentServiceFactoryProxyKey
		] as Comlink.Remote<IDocumentServiceFactoryProxy>;
		assert(
	...
	public async createDocumentService(
		...
		const outerDocumentServiceProxyId = await this.outerProxy.createDocumentService(
			MakeThinProxy(resolvedUrl),
		);

		const clients = await this.outerProxy.clients;
		return InnerDocumentService.create(
			clients[outerDocumentServiceProxyId],
			resolvedUrl,
			logger,
		);
	}
