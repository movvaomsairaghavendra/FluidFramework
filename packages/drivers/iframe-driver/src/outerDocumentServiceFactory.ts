...
const socketIOEvents = ["op", "nack", "pong", "disconnect", "signal"];
...
export class DocumentServiceFactoryProxy implements IDocumentServiceFactoryProxy {
	...
	public get clients() {
		return Comlink.proxy(this._clients);
	}
	...
	public async createDocumentService(
		...
		return this.getDocumentServiceProxy(
	...
	public async createContainer(
		...
		const outerProxyLogger = ChildLogger.create(undefined, "OuterProxyIFrameDriver");
		const connectedDocumentService: IDocumentService =
			await this.documentServiceFactory.createContainer(
				createNewSummary,
				resolvedUrl,
				outerProxyLogger,
			);
	...
	public createProxy(): IDocumentServiceFactoryProxy {
		const proxy: IDocumentServiceFactoryProxy = {
			...
			createDocumentService: Comlink.proxy(async (resolvedUrl) =>
				this.createDocumentService(resolvedUrl),
			),
			createContainer: Comlink.proxy(async (createNewSummary, resolvedUrl) => {
	...
	private getOuterDocumentDeltaConnection(deltaStream: IDocumentDeltaConnection) {
		...
		const connection = {
			...
			get initialClients() {
				return deltaStream.initialClients;
			},
			get initialMessages() {
				return deltaStream.initialMessages;
			},
			get initialSignals() {
				return deltaStream.initialSignals;
			},
		...
		handshake.promise.then(
			(innerProxy: { forwardEvent(event: string, args: any[]): Promise<void> }) => {
