...
describeFullCompat("Detached Container", (getTestObjectProvider) => {
	...
	it("Fire ops during container attach for shared string", async () => {
		...
		(container.deltaManager as any).submit = (type, contents, batch, metadata) => {
			...
			assert.strictEqual(
				contents.contents.contents.content.address,
				sharedStringId,
				"Address should be shared string",
			);
			assert.strictEqual(
				JSON.stringify(contents.contents.contents.content.contents),
				JSON.stringify(ops),
				"Ops should be equal",
			);
			defPromise.resolve();
	...
	it("Fire channel attach ops during container attach", async () => {
		...
		(container.deltaManager as any).submit = (type, contents, batch, metadata) => {
			assert.strictEqual(
				contents.contents.contents.content.id,
				testChannelId,
				"Channel id should match",
			);
			assert.strictEqual(
				contents.contents.contents.content.type,
				SharedMap.getFactory().type,
				"Channel type should match",
			);
			assert.strictEqual(
				contents.contents.contents.type,
	...
	it("Fire ops during container attach for shared ink", async () => {
		...
		(container.deltaManager as any).submit = (type, contents, batch, metadata) => {
			...
			assert.strictEqual(
				contents.contents.contents.content.contents.type,
				"createStroke",
				"Op type should be same",
			);
			assert.strictEqual(
				contents.contents.contents.content.contents.pen.thickness,
				20,
	...
	it("Fire ops during container attach for sparse matrix", async () => {
		...
		(container.deltaManager as any).submit = (type, contents, batch, metadata) => {
			...
			if (
				contents.contents.contents.content.contents.ops[0].type === MergeTreeDeltaType.INSERT
			) {
				assert.strictEqual(
					JSON.stringify(contents.contents.contents.content.contents.ops[0].seg),
					JSON.stringify(seg),
					"Seg should be same",
				);
			} else {
				assert.strictEqual(
					JSON.stringify(contents.contents.contents.content.contents.ops[1].seg),
					JSON.stringify(seg),
					"Seg should be same",
				);
			}
