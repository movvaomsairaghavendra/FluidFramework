...
describeFullCompat("blobs", (getTestObjectProvider) => {
	...
	it("uploadBlob() rejects when runtime is disposed", async () => {
		...
		const blobOpP = new Promise<void>((resolve) =>
			container.deltaManager.on("submitOp", (op) => {
...
describeNoCompat("Garbage collection of blobs", (getTestObjectProvider) => {
	...
	const tests = (deleteUnreferencedContent: boolean = false) => {
		...
		it("collects blobs uploaded in detached and de-duped in attached container", async () => {
			...
			const blobContents = "Blob contents";
			const localHandle = await defaultDataStore._context.uploadBlob(
				stringToBuffer(blobContents, "utf-8"),
			);
			...
			const storageHandle = await defaultDataStore._context.uploadBlob(
				stringToBuffer(blobContents, "utf-8"),
			);
			...
			const s1 = await getUnreferencedNodeStates();
			assert(
				s1.get(localHandle.absolutePath) === "referenced",
				"local id blob should be referenced",
			);
			assert(
				s1.get(storageHandle.absolutePath) === "referenced",
				"storage id blob should also be referenced",
			);
			...
			const s2 = await getUnreferencedNodeStates();
			assert(
				s2.get(localHandle.absolutePath) === "unreferenced",
				"local id blob should still be unreferenced",
			);
			assert(
				s2.get(storageHandle.absolutePath) === "referenced",
				"storage id blob should also be referenced",
			);
			...
			const s3 = await getUnreferencedNodeStates();
			assert(
				s3.get(storageHandle.absolutePath) === "unreferenced",
				"storage id blob should be unreferenced",
			);
			if (deleteUnreferencedContent) {
				assert(
					s3.get(localHandle.absolutePath) === undefined,
					"local id blob should not have a GC entry",
				);
			} else {
				assert(
					s3.get(localHandle.absolutePath) === "unreferenced",
					"local id blob should be unreferenced",
				);
			}
			...
			if (deleteUnreferencedContent) {
				assert(
					s4.get(localHandle.absolutePath) === undefined,
					"local id blob should not have a GC entry",
				);
				assert(
					s4.get(storageHandle.absolutePath) === undefined,
					"storage id blob should not have a GC entry",
				);
			} else {
				assert(
					s4.get(localHandle.absolutePath) === "referenced",
					"local id blob should be re-referenced",
				);
				assert(
					s4.get(storageHandle.absolutePath) === "referenced",
		...
		it("collects blobs uploaded and de-duped in detached container", async () => {
			...
			assert(
				s1.get(storageHandle.absolutePath) === "referenced",
				"storage id blob should be referenced",
			);
			...
			assert(
				s3.get(storageHandle.absolutePath) === "referenced",
				"storage id blob should be referenced",
			);
			if (deleteUnreferencedContent) {
				assert(
			...
			const s4 = await getUnreferencedNodeStates();
			assert(
				s4.get(storageHandle.absolutePath) === "unreferenced",
				"storage id blob should be unreferenced",
			);
			if (deleteUnreferencedContent) {
				assert(
					s4.get(localHandle1.absolutePath) === undefined,
					"local id blob 1 should not have a GC entry",
				);
				assert(
					s4.get(localHandle2.absolutePath) === undefined,
					"local id blob 2 should not have a GC entry",
				);
			} else {
				assert(
					s4.get(localHandle1.absolutePath) === "unreferenced",
					"local id blob 1 should be unreferenced",
				);
				assert(
					s4.get(localHandle2.absolutePath) === "unreferenced",
					"local id blob 2 should be unreferenced",
				);
			}
		});
	};
