...
describeNoCompat("Named root data stores", (getTestObjectProvider) => {
	...
	const anyDataCorruption = async (containers: IContainer[]) =>
		Promise.race(
			containers.map(
				async (c) =>
	...
	const createDataStoreWithProps = async (
	...
	const corruptedAPIAliasOp = async (
	...
	describe("Name conflict expected failures", () => {
		...
		itExpects(
			"Root datastore creation fails at attach op",
			[
				{
					eventName: "fluid:telemetry:Container:ContainerClose",
					error: "Duplicate DataStore created with existing id",
				},
			],
			async () => {
		...
		itExpects(
			"Root datastore creation with props fails at attach op",
			[
				{
					eventName: "fluid:telemetry:Container:ContainerClose",
					error: "Duplicate DataStore created with existing id",
		...
		itExpects(
			"Root datastore creation with the same id breaks container",
			[
				{
					eventName: "fluid:telemetry:Container:ContainerClose",
					error: "Duplicate DataStore created with existing id",
		...
		itExpects(
			"Root datastore creation with the same id and legacy API breaks container",
			[
				{
					eventName: "fluid:telemetry:Container:ContainerClose",
					error: "Duplicate DataStore created with existing id",
		...
		it("Root datastore creation with aliasing turned on throws exception", async () => {
			...
			await setupContainers(testContainerConfig, {
				"Fluid.ContainerRuntime.UseDataStoreAliasing": "true",
			});
			...
			assert.deepEqual(error.getTelemetryProperties().alias, {
				value: "2",
	...
	describe("Aliasing", () => {
		...
		itExpects(
			"Creating a root data store with an existing alias as an id breaks the container",
			[
				{
					eventName: "fluid:telemetry:Container:ContainerClose",
					error: "Duplicate DataStore created with existing id",
