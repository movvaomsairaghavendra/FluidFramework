...
describeNoCompat("Message size", (getTestObjectProvider) => {
	...
	itExpects(
		"A large op will close the container with chunking disabled",
		...
		async () => {
			...
			assert.deepEqual(error.getTelemetryProperties().limit, {
	...
	it("Batched small ops pass when batch is larger than max op size", async function () {
		...
		await setupContainers(
			{ ...testContainerConfig, runtimeOptions: { flushMode: FlushMode.Immediate } },
			{},
		);
