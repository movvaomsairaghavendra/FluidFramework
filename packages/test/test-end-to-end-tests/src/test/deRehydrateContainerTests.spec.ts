...
describeFullCompat(`Dehydrate Rehydrate Container Test`, (getTestObjectProvider) => {
	...
	const assertChannelsTree = (rootOrDatastore: ISnapshotTreeWithBlobContents) =>
		disableIsolatedChannels ? rootOrDatastore : assertSubtree(rootOrDatastore, ".channels");
	...
	const tests = () => {
		it("Dehydrated container snapshot", async () => {
			...
			assert(
				snapshotTree.trees[".protocol"].blobsContents[protocolAttributesBlobId] !== undefined,
		...
		it("Dehydrated container snapshot with dataStore handle stored in map of " +
			"other bound dataStore", async () => {
			const { container, defaultDataStore } =
				await createDetachedContainerAndGetRootDataStore();
		...
		it("Rehydrate container, don't load a data store and then load after container attachment. " +
			"Make changes to dds from rehydrated container and check reflection of changes in other container", async function () {
			if (
		...
		it("Rehydrate container, create but don't load a data store. Attach rehydrated container and load " +
			...
			"make changes to dds in it check reflection of changes in rehydrated container", async function () {
			if (
				this.test?.parent?.title?.includes("N-2") &&
				provider.driver.type === "tinylicious"
			) {
		...
		it("Container rehydration with not bounded dataStore handle stored in root of " +
			"other bounded dataStore", async () => {
			const { container, defaultDataStore } =
				await createDetachedContainerAndGetRootDataStore();
		...
		it("Container rehydration with not bounded dds handle stored in root of bound dataStore. " +
			"The not bounded dds also stores handle not bounded data store", async () => {
			const { container, defaultDataStore } =
				await createDetachedContainerAndGetRootDataStore();
