...
describeFullCompat("Summaries", (getTestObjectProvider) => {
	...
	it("Should generate summary tree with isolated channels disabled", async () => {
		...
		assert(
			stats.blobNodeCount >= 2,
			`Stats expected at least 2 blob nodes, but had ${stats.blobNodeCount}.`,
		);
		// root node, default data store, and default root dds
		assert(
			stats.treeNodeCount >= 3,
			`Stats expected at least 3 tree nodes, but had ${stats.treeNodeCount}.`,
		);

		// Validate summary
		assert(!summary.unreferenced, "Root summary should be referenced.");
		assert(
			summary.tree[channelsTreeName] === undefined,
			"Unexpected .channels tree in summary root.",
		);
		...
		assert(
			attributes.snapshotFormatVersion === "0.1",
			"Datastore attributes snapshotFormatVersion should be 0.1",
		);
		assert(
			attributes.summaryFormatVersion === undefined,
			"Unexpected datastore attributes summaryFormatVersion",
		);
		assert(
			attributes.disableIsolatedChannels === undefined,
			"Unexpected datastore attributes disableIsolatedChannels",
		...
		assert(
			defaultDataStoreNode.tree[channelsTreeName] === undefined,
