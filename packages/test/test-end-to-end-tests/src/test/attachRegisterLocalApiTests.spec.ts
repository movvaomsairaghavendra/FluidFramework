...
describeNoCompat(`Attach/Bind Api Tests For Attached Container`, (getTestObjectProvider) => {
	...
	it("Sticking handle in attached dds should attach the DDS in attached container", async () => {
		...
		assert.strictEqual(rootOfDataStore2.isAttached(), true, "Root Channel should be attached");
		assert.strictEqual(
	...
	it("Stick handle of 2 dds(of 2 different dataStores) in each other and then attaching 1 DDS should " +
		"attach other DDS and dataStore with correct recursion", async () => {
		const { container, defaultDataStore } = await createDetachedContainerAndGetRootDataStore();
		await container.attach(request);
	...
	it("Stick handle of 2 different dataStores and dds in each other and then attaching 1 dataStore should " +
		"attach other dataStores and dds with correct recursion", async () => {
		const { container, defaultDataStore } = await createDetachedContainerAndGetRootDataStore();
		await container.attach(request);
	...
	it("Generate more than 1 dds of a dataStore and then stick handles in different dds and then attaching " +
		"1 handle should attach entire graph", async () => {
		const { container, defaultDataStore } = await createDetachedContainerAndGetRootDataStore();
		await container.attach(request);
		...
		assert.strictEqual(
			...
			true,
			"Test Channel 12 should be bound",
		);
		assert.strictEqual(
			channel2OfDataStore2.isAttached(),
			true,
			"Test Channel 22 should be bound",
		);
		assert.strictEqual(
			channel1OfDataStore3.isAttached(),
			true,
			"Test Channel 13 should be bound",
		);
		assert.strictEqual(
			channel2OfDataStore3.isAttached(),
			true,
			"Test Channel 23 should be bound",
		);
		assert(
			dataStore2.runtime.attachState !== AttachState.Detached,
		...
		assert.strictEqual(
			...
			true,
			"Test Channel 14 should be bound",
		);
	});
