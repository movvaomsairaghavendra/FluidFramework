		...
		while (!done) {
			...
			if (faultInjectionMaxMs !== undefined) {
				...
				assert(
					...
					"faultInjectionMinMs must be greater than or equal to zero.",
				);
				assert(faultInjectionMaxMs > 0, "faultInjectionMaxMs must be greater than zero.");
				assert(
					faultInjectionMaxMs >= faultInjectionMinMs,
		...
		setTimeout(() => {
			if (
				...
				if (deltaConn !== undefined) {
					...
					const canRetry =
						random.integer(0, runConfig.testConfig.numClients - 1)(runConfig.randEng) === 0
							? false
							: true;
