...
describe("generators", () => {
	...
	function computeChiSquared<T>(weights: [T, number][], sampleCounts: Counter<T>): number {
		const numberOfSamples = Array.from(sampleCounts.values()).reduce(
