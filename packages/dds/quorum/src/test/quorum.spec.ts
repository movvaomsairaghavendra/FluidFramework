...
function createConnectedQuorum(
	id: string,
...
describe("Quorum", () => {
	...
	describe("Disconnect/Reconnect", () => {
		...
		it("Doesn't resubmit unsequenced proposals that were sent before offline but are futile after reconnect", async () => {
			...
			containerRuntimeFactory.processAllMessages();
			assert.strictEqual(
				quorum2.get(targetKey),
				"expected",
				"Quorum2 should see the expected value",
			);
			assert.strictEqual(
				quorum1.get(targetKey),
				undefined,
				"Quorum1 should not see any value",
			);
			...
			assert.strictEqual(
				quorum1.get(targetKey),
				"expected",
				"Quorum1 should see the expected value",
			);
		});
		...
		it("Sequenced proposals that remained pending during offline have correct state after reconnect", async () => {
			...
			assert.strictEqual(
				quorum1.get(targetKey),
				undefined,
				"Quorum1 should not see the expected value",
			);
			assert.strictEqual(
				quorum2.get(targetKey),
				undefined,
				"Quorum2 should not see the expected value",
			);
			containerRuntimeFactory.processAllMessages();
			assert.strictEqual(
				quorum1.get(targetKey),
