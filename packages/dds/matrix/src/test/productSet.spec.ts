...
describe("Product-set tests", () => {
	...
	const createBsp = createFromKey(
		boundedParetoSetOperations(
			1,
			anyValue[0] + 1,
			anyValue[1],
			anyValue,
			"Interval operations" as const,
		),
	);

	const toProd = ({ xDim, yDim }: Rect2D) =>
		createFromProduct({ xDim: createBsp(xDim), yDim: createBsp(yDim) });
	...
	it("Compute a large set difference", () => {
		...
		for (let i = 3; i < 7; i += 1) {
			for (let j = 3; j < 7; j += 1) {
				rectangles.push({
					xDim: [i, Math.max(8, i + j)],
					yDim: [j + 2, Math.max(9, i - j + 10)],
				});
				rectangles.push({
					xDim: [i + 2, Math.max(8, i + j)],
					yDim: [j, Math.max(9, i - j + 10)],
				});
			}
		}
	...
	it("Symmetric difference", () => {
		...
		const expected = points
			.map(([x, y]) => toProd({ xDim: [x, x + 1], yDim: [y, y + 1] }))
			.reduce(unionProduct);
	...
	it("Test for-each", () => {
		...
		const u = unionProduct(
			unionProduct(
				toProd({ xDim: [5, 10], yDim: [5, 10] }),
				toProd({ xDim: [10, 15], yDim: [5, 10] }),
			),
			unionProduct(
				toProd({ xDim: [5, 10], yDim: [10, 15] }),
				toProd({ xDim: [12, 17], yDim: [12, 17] }),
			),
		);
