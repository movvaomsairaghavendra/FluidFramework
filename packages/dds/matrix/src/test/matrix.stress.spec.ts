...
describe("Matrix", () => {
	describe("stress", () => {
		...
		async function stress(
			...
			try {
				...
				const setCells = (
					...
					trace?.push(
						`matrix${matrixIndex + 1}.setCells(/* row: */ ${row}, /* col: */ ${col}, /* colCount: */ ${colCount}, ${JSON.stringify(values)});    // rowCount: ${matrix.rowCount} colCount: ${matrix.colCount} stride: ${matrix.colCount} length: ${values.length}`,
					);
					...
					switch (int32(7)) {
						case 0: {
							...
							if (rowCount > 0) {
								...
								// eslint-disable-next-line max-len
								trace?.push(
									`matrix${matrixIndex + 1}.removeRows(/* rowStart: */ ${row}, /* rowCount: */ ${numRemoved});    // rowCount: ${matrix.rowCount - numRemoved}, colCount: ${matrix.colCount}`,
								);
						...
						case 1: {
							...
							if (colCount > 0) {
								...
								// eslint-disable-next-line max-len
								trace?.push(
									`matrix${matrixIndex + 1}.removeCols(/* colStart: */ ${col}, /* colCount: */ ${numRemoved});    // rowCount: ${matrix.rowCount}, colCount: ${matrix.colCount - numRemoved}`,
								);
						...
						case 2: {
							...
							// eslint-disable-next-line max-len
							trace?.push(
								`matrix${matrixIndex + 1}.insertRows(/* rowStart: */ ${row}, /* rowCount: */ ${numInserted});    // rowCount: ${matrix.rowCount + numInserted}, colCount: ${matrix.colCount}`,
							);
						...
						case 3: {
							...
							// eslint-disable-next-line max-len
							trace?.push(
								`matrix${matrixIndex + 1}.insertCols(/* colStart: */ ${col}, /* colCount: */ ${numInserted});    // rowCount: ${matrix.rowCount}, colCount: ${matrix.colCount + numInserted}`,
							);
		...
		for (const { numClients, numOps, syncProbability, disconnectProbability, seed } of [
			...
			// eslint-disable-next-line max-len
			it(`Stress (numClients=${numClients} numOps=${numOps} syncProbability=${syncProbability} disconnectProbability=${disconnectProbability} seed=0x${seed.toString(16).padStart(8, "0")})`, // Note: Must use 'function' rather than arrow '() => { .. }' in order to set 'this.timeout(..)'
			async function () {
				this.timeout(20000);
		...
		it("stress-loop", // Note: Must use 'function' rather than arrow '() => { .. }' in order to set 'this.timeout(..)'
		async function () {
