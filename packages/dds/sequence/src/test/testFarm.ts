...
function makeBookmarks(client: TestClient, bookmarkCount: number) {
	...
	for (let i = 0; i < bookmarkCount; i++) {
		...
		if (segoff1?.segment && segoff2?.segment) {
			...
			bookmarks.push(
				new SharedString.SequenceInterval(lref1, lref2, SharedString.IntervalType.Simple),
			);
		} else {
			i--;
		}
...
export function TestPack(verbose = true) {
	...
	function reportTiming(client: TestClient) {
		...
		if (client.localOps > 0) {
			console.log(
				`local time ${client.localTime} us ops: ${client.localOps} ave time ${aveLocalTime}`,
			);
		}
		console.log(`ord time average: ${aveOrdTime}us max ${stats.maxOrdTime}us`);
		console.log(
			`${client.longClientId} accum time ${client.accumTime} us ops: ${client.accumOps} ave time ${aveTime} - wtime ${adjTime} pack ${avePackTime} ave window ${aveWindow}`,
		);
		console.log(
			`${client.longClientId} accum window time ${client.accumWindowTime} us ave window time total ${aveWindowTime} not in ops ${aveExtraWindowTime}; max ${client.maxWindowTime}`,
		);
	}
	...
	function clientServer(startFile?: string, initRounds = 1000) {
		...
		if (testSyncload) {
			...
			const fromLoadText = new MergeTreeTextHelper(fromLoad).getText(
				UniversalSequenceNumber,
				NonCollabClient,
			);
		...
		function checkTextMatch() {
			...
			for (const client of clients) {
				...
				if (cliText !== serverText) {
					console.log(
						`mismatch @${testServer.getCurrentSeq()} client @${client.getCurrentSeq()} id: ${client.getClientId()}`,
					);
		...
		function randomWordMove(client: TestClient) {
			...
			if (word1) {
				...
				const segOff = client.getContainingSegment(pos);
				const insertOp =
					!insertAsRefPos && segOff.segment
						? client.insertAtReferencePositionLocal(
								new MergeTree.LocalReference(
									client,
									segOff.segment,
									segOff.offset,
									MergeTree.ReferenceType.Transient,
								),
				...
				if (!useGroupOperationsForMoveWord) {
					testServer.enqueueMsg(client.makeOpMessage(insertOp));
					if (TestClient.useCheckQ) {
						client.enqueueTestString();
					}
				} else if (insertOp) {
		...
		function finishRound(roundCount: number) {
			...
			if (measureBookmarks) {
					...
					for (let i = 0; i < rangeChecksPerRound; i++) {
						...
						if (segoff1 && segoff2 && segoff1.segment && segoff2.segment) {
							...
							if (segoff1.segment === segoff2.segment) {
								...
								ordErrors++;
								console.log(
									`reverse ordinals ${MergeTree.ordinalToArray(segoff1.segment.ordinal)} > ${MergeTree.ordinalToArray(segoff2.segment.ordinal)}`,
								);
								console.log(
				...
				if (measureRanges) {
					...
					for (let i = 0; i < posChecksPerRound; i++) {
						...
						if (segoff1?.segment && segoff2?.segment) {
							const lrefPos1 = new MergeTree.LocalReference(
								testServer,
								<MergeTree.BaseSegment>segoff1.segment,
								segoff1.offset,
							);
							const lrefPos2 = new MergeTree.LocalReference(
								testServer,
								<MergeTree.BaseSegment>segoff2.segment,
								segoff2.offset,
							);
					...
					for (let i = 0; i < posChecksPerRound; i++) {
						...
						if (showResults) {
							...
							for (const ival of ivals) {
								const pos1 = testServer.mergeTree.referencePositionToLocalPosition(
									ival.key.start,
								);
								const pos2 = testServer.mergeTree.referencePositionToLocalPosition(
									ival.key.end,
								);
            ...
            */
			...
			if (0 === roundCount % 100) {
				...
				console.log(
					`round: ${roundCount} seq ${testServer.seq} char count ${testServer.getLength()} height ${stats.maxHeight} lv ${stats.leafCount} rml ${stats.removedLeafCount} p ${posLeaves} nodes ${stats.nodeCount} pop ${liveAve} histo ${stats.histo}`,
				);
				if (extractSnapOps > 0) {
					aveExtractSnapTime = (extractSnapTime / extractSnapOps).toFixed(1);
				...
				if (measureBookmarks) {
					...
					console.log(
						`bookmark count ${bookmarkCount} ave. per seg ${bookmarksPerSeg} time/read ${timePerRead}`,
					);
					if (measureRanges) {
						...
						console.log(
							`ave. per bookmark context check ${timePerContextCheck} ave results per check ${results}`,
						);
						...
						console.log(
							`ave. per bookmark range check ${timePerRangeCheck} ave results per check ${resultsRange}`,
						);
					}
				}
	...
	function firstTest() {
		...
		cli.insertMarkerRemote(
			0,
			{ refType: MergeTree.ReferenceType.Tile },
		...
		const lref1 = new MergeTree.LocalReference(
			cli,
			<MergeTree.BaseSegment>segoff.segment,
			segoff.offset,
		);
		...
		cli.applyMsg(cli.makeOpMessage(MergeTree.createRemoveRangeOp(3, 5), 8, 6, "1"));
		if (verbose) {
			console.log(cli.mergeTree.toString());
			for (let clientId = 0; clientId < 6; clientId++) {
		...
		cli.applyMsg(cli.makeOpMessage(MergeTree.createRemoveRangeOp(1, 3), 1, 0, "3"));
		if (verbose) {
			console.log(cli.mergeTree.toString());
		}
        ...
        */
		cli.applyMsg(cli.makeOpMessage(MergeTree.createRemoveRangeOp(3, 8), 8, 7, "2"));
		cli.applyMsg(cli.makeOpMessage(MergeTree.createRemoveRangeOp(5, 7), 9, 7, "2"));
		if (verbose) {
			console.log(cli.mergeTree.toString());
			for (let clientId = 0; clientId < 4; clientId++) {
		...
		cli.applyMsg(cli.makeOpMessage(MergeTree.createRemoveRangeOp(3, 6), 10, 9, "2"));
...
export function mergeTreeCheckedTest() {
	...
	for (let i = 0; i < insertCount; i++) {
		...
		if (!checkInsertMergeTree(mergeTree, pos, makeCollabTextSegment(s), true)) {
			console.log(
				`i: ${i} preLen ${preLen} pos: ${pos} slen: ${slen} s: ${s} itree len: ${mergeTree.getLength(UniversalSequenceNumber, LocalClientId)}`,
			);
	...
	for (let i = 0; i < largeRemoveCount; i++) {
		...
		if (!checkMarkRemoveMergeTree(mergeTree, pos, pos + dlen, true)) {
			console.log(
				`i: ${i} preLen ${preLen} pos: ${pos} dlen: ${dlen} itree len: ${mergeTree.getLength(UniversalSequenceNumber, LocalClientId)}`,
			);
			console.log(mergeTree.toString());
			break;
		}
	...
	for (let i = 0; i < removeCount; i++) {
		...
		if (i & 1) {
			if (!checkMarkRemoveMergeTree(mergeTree, pos, pos + dlen, true)) {
				console.log(
					`mr i: ${i} preLen ${preLen} pos: ${pos} dlen: ${dlen} itree len: ${mergeTree.getLength(UniversalSequenceNumber, LocalClientId)}`,
				);
				console.log(mergeTree.toString());
				errorCount++;
				break;
