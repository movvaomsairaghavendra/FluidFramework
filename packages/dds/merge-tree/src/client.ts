...
export class Client {
	...
	public createLocalReferencePosition(
		...
		return this.mergeTree.createLocalReferencePosition(
			segment,
			offset,
			refType,
			properties,
	...
	private completeAndLogOp(
		...
		if (!opArgs.sequencedMessage) {
			...
			assert(
				this.mergeTree.getCollabWindow().minSeq <=
					opArgs.sequencedMessage.minimumSequenceNumber,
	...
	protected findReconnectionPosition(segment: ISegment, localSeq: number) {
		assert(
			localSeq <= this.mergeTree.collabWindow.localSeq,
        ...
        */
		this.mergeTree.walkAllSegments(this.mergeTree.root, (seg) => {
			...
			if (
				(seg.localSeq === undefined || seg.localSeq <= localSeq) && // Is inserted
	...
	public summarize(
		...
		if (this.mergeTree.options?.newMergeTreeSnapshotFormat === true) {
			...
			const snap = new SnapshotV1(this.mergeTree, this.logger, (id) =>
	...
	getStackContext(startPos: number, rangeLabels: string[]): RangeStackMap {
		return this.mergeTree.getStackContext(
			startPos,
	...
	updateConsensusProperty(op: IMergeTreeAnnotateMsg, msg: ISequencedDocumentMessage) {
		...
		this.mergeTree.addMinSeqListener(msg.sequenceNumber, () =>
	...
	getContainingSegment<T extends ISegment>(pos: number, op?: ISequencedDocumentMessage) {
		...
		return this.mergeTree.getContainingSegment<T>(
			pos,
			args.referenceSequenceNumber,
			args.clientId,
		);
	}
