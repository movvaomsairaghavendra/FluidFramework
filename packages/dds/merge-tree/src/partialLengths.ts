...
export class PartialSequenceLengths {
	...
	private static combineBranch(
		...
		for (let i = 0; i < block.childCount; i++) {
			...
			if (!child.isLeaf()) {
				...
				if (recur) {
					childBlock.partialLengths = PartialSequenceLengths.combine(
						mergeTree,
						childBlock,
						collabWindow,
						true,
	...
	private static insertSegment(
		...
		if (removalInfo) {
			...
			removeClientOverlap =
				removalInfo.removedClientIds.length > 1
		...
		if (indexFirstGTE < seqPartialsLen && seqPartials[indexFirstGTE].seq === seq) {
			...
			if (removeClientOverlap) {
				const overlapClients = PartialSequenceLengths.getOverlapClients(
					removeClientOverlap,
					segmentLen,
				);
				pLen = {
	...
	private static addSeq(
		...
		seqPartialLen.len =
			penultPartialLen !== undefined
				? seqPartialLen.seglen + penultPartialLen.len
				: seqPartialLen.seglen;
	}
	public minLength = 0;
