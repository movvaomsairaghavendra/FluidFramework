...
function assertLocalReferences(
	lref: ReferencePosition | LocalReference,
...
export class LocalReferenceCollection {
	...
	public addLocalRef(lref: LocalReference | ReferencePosition) {
		...
		const refsAtOffset = (this.refsByOffset[lref.offset] = this.refsByOffset[lref.offset] ?? {
			at: ListMakeHead(),
		});
		const atRefs = (refsAtOffset.at = refsAtOffset.at ?? ListMakeHead());
	...
	public addBeforeTombstones(...refs: Iterable<LocalReference | ReferencePosition>[]) {
		...
		if (!beforeRefs.empty() && this.refsByOffset[0]?.before === undefined) {
			const refsAtOffset = (this.refsByOffset[0] = this.refsByOffset[0] ?? {
				before: beforeRefs,
			});
	...
	public addAfterTombstones(...refs: Iterable<LocalReference | ReferencePosition>[]) {
		...
		if (!afterRefs.empty() && this.refsByOffset[lastOffset]?.after === undefined) {
			const refsAtOffset = (this.refsByOffset[lastOffset] = this.refsByOffset[lastOffset] ?? {
				after: afterRefs,
			});
