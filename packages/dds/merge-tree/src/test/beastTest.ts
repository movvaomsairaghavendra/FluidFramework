...
import { KeyComparer, Property, PropertyAction, SortedDictionary } from "../base";
import { ProxString, RedBlackTree, Stack, TST } from "../collections";
import {
	LocalClientId,
...
import { IJSONTextSegment, MergeTreeTextHelper, TextSegment } from "../textSegment";
...
export function TestPack(verbose = true) {
		...
		function finishRound(roundCount: number) {
            ...
            */
			...
			if (0 === roundCount % 100) {
				...
				log(
					`round: ${roundCount} seq ${server.seq} char count ${server.getLength()} height ${stats.maxHeight} lv ${stats.leafCount} rml ${stats.removedLeafCount} p ${posLeaves} nodes ${stats.nodeCount} pop ${liveAve} histo ${stats.histo}`,
				);
				if (extractSnapOps > 0) {
					aveExtractSnapTime = (extractSnapTime / extractSnapOps).toFixed(1);
	...
	function firstTest() {
		...
		const segs = <SharedStringJSONSegment[]>(
			new SnapshotLegacy(cli.mergeTree, DebugLogger.create("fluid:snapshot")).extractSync()
		);
		if (verbose) {
