...
export const defaultOptions: IMergeTreeOperationRunnerConfig & {
	minLength: number;
...
describe("MergeTree.Client", () => {
	...
	doOverRange(opts.clients, opts.growthFunc.bind(opts), (clientCount) => {
		it(`applyStashedOpFarm_${clientCount}`, async () => {
			...
			seq = runMergeTreeOperationRunner(mt, seq, clients, opts.minLength, opts, (s, m, c) =>
				applyMessagesWithReconnect(s, m, c, stashClients),
			);
		}).timeout(30 * 1000);
	});
});
