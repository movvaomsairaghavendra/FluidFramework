...
import { Comparer, Heap, List, ListMakeHead, Stack } from "./collections";
import {
	LocalClientId,
	NonCollabClient,
...
export class MergeBlock extends MergeNode implements IMergeBlock {
	...
	public setOrdinal(child: IMergeNode, index: number) {
		...
		assert(
			child.ordinal.length === this.ordinal.length + 1,
...
export class Marker extends BaseSegment implements ReferencePosition {
	...
	hasSimpleType(simpleTypeName: string) {
		return (
			!!this.properties && this.properties[reservedMarkerSimpleTypeKey] === simpleTypeName
		);
	}
...
function tileShift(
	...
	if (node.isLeaf()) {
		...
		const block = <IHierBlock>node;
		let marker: Marker;
		marker = searchInfo.posPrecedesTile
...
export class MergeTree {
	 ...
	 */
	public _getSlideToSegment(segoff: {
	...
	public resolveRemoteClientPosition(
		...
		if (segmentInfo && segmentInfo.segment) {
			const segmentPosition = this.getPosition(
				segmentInfo.segment,
				segwindow.currentSeq,
				segwindow.clientId,
			);
	...
	private insertChildNode(block: IMergeBlock, child: IMergeNode, childIndex: number) {
		assert(
			block.childCount < MaxNodesInBlock,
	...
	public markRangeRemoved(
		...
		this.mapRange(
			{ leaf: markRemoved, post: afterMarkRemoved },
			refSeq,
			clientId,
			undefined,
			start,
	...
	private nodeMapReverse(
		block: IMergeBlock,
		leafAction: ISegmentAction<undefined>,
		pos: number,
		refSeq: number,
		clientId: number,
	) {
