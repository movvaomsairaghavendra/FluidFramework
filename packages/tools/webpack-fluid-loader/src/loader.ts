...
function wrapWithRuntimeFactoryIfNeeded(
	...
	if (fluidModuleExport.IRuntimeFactory === undefined) {
		...
		const runtimeFactory = new ContainerRuntimeFactoryWithDefaultDataStore(
			defaultFactory,
			new Map([[defaultFactory.type, Promise.resolve(defaultFactory)]]),
		);
		return {
			module: {
				fluidExport: {
...
async function createWebLoader(
	...
	return new Loader({
		urlResolver: testOrderer
			? new MultiUrlResolver(documentId, window.location.origin, options, true)
			: urlResolver,
	...
	if (manualAttach) {
		...
		serializeButton.onclick = () => {
			...
			rehydrateButton.onclick = async () => {
				...
				const newLeftDiv: HTMLDivElement =
