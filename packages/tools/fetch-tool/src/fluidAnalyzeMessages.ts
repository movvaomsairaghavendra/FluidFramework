...
function dumpStats(
	...
	if (props.removeTotals === undefined) {
		if (allOtherCount || allOtherSize) {
			...
			console.log(
				`${`All Others (${sorted.length - recordsToShow})`.padEnd(nameLength)} │ ${formatNumber(allOtherCount).padStart(fieldSizes[0])} ${formatNumber(allOtherSize).padStart(fieldSizes[1])}`,
			);
		}
		console.log(`${"─".repeat(nameLength + 1)}┼${"─".repeat(fieldsLength + 1)}`);
		// eslint-disable-next-line max-len
		console.log(
			`${"Total".padEnd(nameLength)} │ ${formatNumber(totalCount).padStart(fieldSizes[0])} ${formatNumber(sizeTotal).padStart(fieldSizes[1])}`,
		);
	}
}
...
function processDataStoreAttachOp(
	...
	const parsedAttachMessage: IAttachMessage =
