...
describe("Error Logging", () => {
	...
	describe("isTaggedTelemetryPropertyValue", () => {
		...
		it("non-object value ok", () => {
			...
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ tag: "any string" }),
				true,
			...
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: Symbol("okay"), tag: "any string" }),
				true,
			);
		});
		it("object or null value not ok", () => {
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: { foo: "bar" }, tag: "any string" }),
				false,
				"object value not ok",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: {}, tag: "any string" }),
				false,
				"object value not ok",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: null, tag: "any string" }),
				false,
				"null value not ok",
			);
		});
		it("non-string tag not ok", () => {
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: "hello", tag: 1 }),
				false,
				"number tag is bad",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: "hello", tag: false }),
				false,
				"boolean tag is bad",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: "hello", tag: {} }),
				false,
				"object tag is bad",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: "hello", tag: null }),
				false,
				"null tag is bad",
			);
			assert.strictEqual(
				isTaggedTelemetryPropertyValue({ value: "hello" }),
				false,
	...
	describe("LoggingError", () => {
		it("ctor props are assigned to the object", () => {
			const loggingError = new LoggingError("myMessage", {
				...
				p2: "two",
				p3: true,
				tagged: { value: 4, tag: "PackageData" },
			});
		...
		const untrustedInputs: {
			...
			"undef": () => ({
				...
				expectedOutput: typicalOutput(
					"undefined",
					"<<natural stack>>",
				).withExpectedTelemetryProps({ typeofError: "undefined" }),
			}),
			"false": () => ({
				input: false,
			...
			"symbol": () => ({
				...
				expectedOutput: typicalOutput(
					"Symbol(Unique)",
					"<<natural stack>>",
				).withExpectedTelemetryProps({ typeofError: "symbol" }),
			}),
			"function": () => ({
				input: () => {},
				expectedOutput: typicalOutput(
					"() => { }",
					"<<natural stack>>",
				).withExpectedTelemetryProps({ typeofError: "function" }),
			}),
			"emptyArray": () => ({
				input: [],
...
describe("Error Discovery", () => {
	...
	it("isValidLegacyError", () => {
		...
		assert(
			isValidLegacyError(
				Object.assign(createExternalError("hello"), { errorType: "someErrorType" }),
			),
		);
	});
