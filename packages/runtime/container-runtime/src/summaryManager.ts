...
type StopReason = Extract<
	...
	"parentNotConnected" | "parentShouldNotSummarize"
>;
...
export class SummaryManager implements IDisposable {
	...
	private getShouldSummarizeState(): ShouldSummarizeState {
		...
		if (
			this.connectedState.clientId !== this.clientElection.electedParentId ||
		...
		this.delayBeforeCreatingSummarizer()
			.then(async (startWithInitialDelay: boolean) => {
				...
				if (
					...
					this.getShouldSummarizeState().shouldSummarize === false
				) {
					return "early exit";
				}
