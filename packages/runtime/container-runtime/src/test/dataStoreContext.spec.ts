...
import { LocalFluidDataStoreContext, RemoteFluidDataStoreContext } from "../dataStoreContext";
...
describe("Data Store Context Tests", () => {
	...
	describe("RemoteDataStoreContext", () => {
		...
		describe("Initialization - can correctly initialize and generate attributes", () => {
			 ...
			 */
			function testGenerateAttributes(
				writeIsolatedChannels: boolean,
				...
				it("can read from latest with isolated channels", async () =>
					testGenerateAttributesCore(true, {
						pkg: JSON.stringify([pkgName]),
						summaryFormatVersion: 2,
						isRootDataStore: true,
					}));

				it("can read from latest without isolated channels", async () =>
					testGenerateAttributesCore(false, {
						pkg: JSON.stringify([pkgName]),
						summaryFormatVersion: 2,
						isRootDataStore: true,
				...
				it("can read from previous snapshot format", async () =>
					testGenerateAttributesCore(false, {
						pkg: JSON.stringify([pkgName]),
				...
				it("can read from oldest snapshot format", async () =>
					testGenerateAttributesCore(false, {
						pkg: pkgName,
					}));
			}
			...
			describe("writing with isolated channels disabled", () =>
				testGenerateAttributes(false /* writeIsolatedChannels */, {
					pkg: JSON.stringify([pkgName]),
					snapshotFormatVersion: "0.1",
					isRootDataStore: true,
				}));
			...
			describe("writing with isolated channels enabled", () =>
				testGenerateAttributes(true /* writeIsolatedChannels */, {
					pkg: JSON.stringify([pkgName]),
					summaryFormatVersion: 2,
					isRootDataStore: true,
				}));
