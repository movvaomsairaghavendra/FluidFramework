...
describe("Garbage Collection Tests", () => {
	...
	describe("Session expiry", () => {
		...
		const createGCOverride = (metadata?: IContainerRuntimeMetadata) => {
			return createGarbageCollector({ metadata }, undefined /* gcBlobsMap */, () => {
				closeCalled = true;
			});
		};
		...
		it("Session expires for a new container", async () => {
			...
			assert(
				...
				"Close should have been called at exact expiry.",
			);
		});
		...
		it("Session expiry disabled via DisableSessionExpiry config", async () => {
			...
			assert(
				!closeCalledAfterExactTicks(defaultSessionExpiryDurationMs),
				"Close should NOT have been called due to disable.",
			);
		});
		...
		it("Session expiry overridden via TestOverride setting (existing container)", async () => {
			...
			createGCOverride(metadata);
			assert(
				closeCalledAfterExactTicks(customExpiryMs),
				"Close should have been called at exact expiry.",
			);
		});
		...
		it("Session expiry override ignored if RunSessionExpiry setting disabled", async () => {
			...
			assert(
				!closeCalled,
				"Close should not have been called since runSessionExpiry disabled.",
			);
		...
		it("Session expiry override ignored if DisableSessionExpiry is true", async () => {
			...
			assert(
				!closeCalled,
				"Close should not have been called since DisableSessionExpiry true.",
			);
			clock.tick(defaultSessionExpiryDurationMs);
			assert(
				!closeCalled,
