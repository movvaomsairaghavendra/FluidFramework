...
export class GarbageCollector implements IGarbageCollector {
	...
	protected constructor(createParams: IGarbageCollectorCreateParams) {
		...
		if (
			this.sessionExpiryTimeoutMs !== undefined &&
			...
			if (createParams.snapshotCacheExpiryMs !== undefined) {
				this.sweepTimeoutMs =
					this.sessionExpiryTimeoutMs + createParams.snapshotCacheExpiryMs + oneDayMs;
			}
		}
		...
		const baseSummaryStateP = new LazyPromise<IGarbageCollectionState | undefined>(
			async () => {
				...
				if (gcSnapshotTree !== undefined) {
					...
					if (this.trackGCState) {
						this.latestSerializedSummaryState = JSON.stringify(
							generateSortedGCState(baseGCState),
						);
					}
					return baseGCState;
				}
				...
				for (const [dsId, dsSnapshotTree] of Object.entries(dataStoreSnapshotTree.trees)) {
					...
					const gcSummaryDetails =
						await readAndParseBlob<IGarbageCollectionDetailsBase>(blobId);
		...
		this.baseGCDetailsP = new LazyPromise<Map<string, IGarbageCollectionDetailsBase>>(
			async () => {
				const baseState = await baseSummaryStateP;
				if (baseState === undefined) {
					return new Map();
	...
	public async collectGarbage(options: {
		...
		const { fullGC = this.gcOptions.runFullGC === true || this.summaryStateNeedsReset } =
			options;
	...
	private generateStats(gcResult: IGCResult): IGCStats {
		...
		const updateNodeStats = (nodeId: string, referenced: boolean) => {
			...
			const stateUpdated =
				this.previousGCDataFromLastRun === undefined ||
