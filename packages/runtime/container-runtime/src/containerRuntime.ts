...
export function unpackRuntimeMessage(message: ISequencedDocumentMessage) {
	if (message.type === MessageType.Operation) {
		...
		assert(
			isUnpackedRuntimeMessage(message),
		...
		ISummarizerInternalsProvider
{
	...
	private isSummarizerClientElectionEnabled(): boolean {
		if (this.mc.config.getBoolean("Fluid.ContainerRuntime.summarizerClientElection")) {
			return (
				this.mc.config.getBoolean("Fluid.ContainerRuntime.summarizerClientElection") ?? true
			);
		}
	...
	private constructor(
		...
		this.disableIsolatedChannels =
			this.runtimeOptions.summaryOptions.disableIsolatedChannels ?? false;
		...
		this._maxOpSizeInBytes =
			this.mc.config.getNumber(maxOpSizeInBytesKey) ?? defaultMaxOpSizeInBytes;
		...
		this.opTracker = new OpTracker(
			this.deltaManager,
			this.mc.config.getBoolean(disableOpTrackingKey) === true,
		);
	}
	...
	private async getDataStoreFromRequest(id: string, request: IRequest): Promise<IFluidRouter> {
		const wait =
			typeof request.headers?.[RuntimeHeaders.wait] === "boolean"
	 ...
	 */
	private async createRootDataStoreLegacy(
		pkg: string | string[],
		rootDataStoreId: string,
	): Promise<IFluidRouter> {
		const fluidDataStore = await this._createDataStore(
			pkg,
			true /* isRoot */,
			rootDataStoreId,
		);
	 ...
	 */
	private async createAndAliasDataStore(
		pkg: string | string[],
		alias: string,
		props?: any,
	): Promise<IDataStore> {
	...
	public async submitSummary(options: ISubmitSummaryOptions): Promise<SubmitSummaryResult> {
		...
		try {
			...
			const dataStoreTree = this.disableIsolatedChannels
				? summaryTree
				: summaryTree.tree[channelsTreeName];
	...
	private addChunk(clientId: string, chunkedContent: IChunkedOp) {
		...
		assert(
			chunkedContent.chunkId === map.length + 1,
	...
	private submitMaybeChunkedMessages(
		...
		if (this._maxOpSizeInBytes >= 0) {
			...
			this.closeFn(
				new GenericError("OpTooLarge", /* error */ undefined, {
					length: {
	...
	private async refreshLatestSummaryAckFromServer(
		...
		const { snapshotTree, versionId } = await this.fetchSnapshotFromStorage(
			null,
			summaryLogger,
			{
				eventName: "RefreshLatestSummaryGetSnapshot",
	...
	public async getSnapshotBlobs(): Promise<void> {
		if (
			!(
				this.mc.config.getBoolean("enableOfflineLoad") ?? this.runtimeOptions.enableOfflineLoad
			) ||
			this.attachState !== AttachState.Attached ||
