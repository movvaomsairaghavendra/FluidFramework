	...
	implements IFluidDataStoreContext, IDisposable
{
		...
		if (!this.channelDeferred) {
			...
			this.realizeCore(this.existing).catch((error) => {
				...
				errorWrapped.addTelemetryProperties({
					fluidDataStoreId: { value: this.id, tag: "PackageData" },
				});
	...
	private async summarizeInternal(
		...
		if (!this.writeGCDataAtRoot) {
			addBlobToSummary(
				summarizeResult,
				gcBlobKey,
				JSON.stringify(this.summarizerNode.getGCSummaryDetails()),
			);
		}
	...
	private updateChannelUsedRoutes() {
		...
		const usedChannelRoutes = this.lastUsedState.usedRoutes.filter((id: string) => {
			return id !== "/" && id !== "";
		});
...
export class RemoteFluidDataStoreContext extends FluidDataStoreContext {
	...
	private readonly initialSnapshotDetailsP = new LazyPromise<ISnapshotDetails>(async () => {
		...
		if (!!tree && tree.blobs[dataStoreAttributesBlobName] !== undefined) {
			...
			const attributes = await localReadAndParse<ReadFluidDataStoreAttributes>(
