...
export class BlobManager {
	...
	private load(snapshot: IBlobManagerLoadInfo): void {
		if (snapshot.ids) {
			...
			snapshot.ids.map((entry) =>
				detached ? this.detachedBlobIds.add(entry) : this.blobIds.add(entry),
			);
		}
	...
	public summarize(telemetryContext?: ITelemetryContext): ISummaryTreeWithStats {
		...
		const attachingOrAttached =
			!!this.redirectTable || this.runtime.attachState !== AttachState.Detached;
